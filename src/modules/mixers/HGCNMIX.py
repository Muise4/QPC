...
class HGCNMixer(nn.Module):
    def __init__(self, args):
        ...
        self.indiv_u_dim = int(np.prod(args.observation_shape)) #个人观测维度
        self.use_one_hot = False
        self.n_hyper_edge = self.hyper_edge_num #将超边数量固定了
        if self.use_one_hot:
            self.n_hyper_edge += self.n_agents
        self.use_elu = True
        self.hyper_edge_net = nn.Sequential(
            nn.Linear(in_features=self.indiv_u_dim, out_features=self.hyper_edge_num), #由观测生成超边数量的参数
            nn.ReLU(),
        )
        self.hidden_dim = 64
    ...
    def forward(self, agent_qs, states, indiv_us):    #选定的动作，状态，观测
        bs = agent_qs.size(0)
        sl = agent_qs.size(1)
        agent_qs = agent_qs.view(-1, agent_qs.size(-1))
        indiv_us = indiv_us.reshape(-1, indiv_us.size(-2), indiv_us.size(-1))
        hyper_graph = self.build_hyper_net(indiv_us)    #由观测构建超图,行是节点，列是超边，超边完全由节点的观测生成
        states = states.reshape(-1, states.size(-1))
        hyper_graph = hyper_graph.reshape(-1, hyper_graph.size(-2), hyper_graph.size(-1))   ##(bs * sl, n_agents, n_edges)
        node_features = agent_qs.unsqueeze(dim=-1)  #(bs * sl, n_agents, 1)
