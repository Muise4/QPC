...
from envs.SSD.env.agent import CleanupAgent
from envs.SSD.env.gym.discrete_with_dtype import DiscreteWithDType
from envs.SSD.env.map_env import MapEnv
from envs.SSD.maps import CLEANUP_MAP
...
CLEANUP_COLORS = {
    b"C": np.array([100, 255, 255], dtype=np.uint8),  # Cyan cleaning beam,清理光线
...
SPAWN_PROB_COM = [0, 0.0025, 0.01, 0.025]
...
appleRespawnProbability_com = 0.025
...
class CleanupEnv(MapEnv):
    def __init__(
        ...
        if not self.more_com:
            self.current_apple_spawn_prob = appleRespawnProbability
        elif self.more_com:
            self.current_apple_spawn_prob = appleRespawnProbability_com
